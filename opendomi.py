...
def check_soa(ns):
    ...
    soa_error=[]
    for index, field in enumerate(('refresh', 'retry', 'expire', 'ttl'), 3):
        soa_value = soa_fields[index]
        def_value = default_soa[field]
        if soa_value * 20 > def_value or soa_value / 20 < def_value:
            soa_error.append('%s: %s' % (field, soa_value))
    if soa_error:
